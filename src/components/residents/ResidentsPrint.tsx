...
export default function ResidentsPrint({
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      alert(language === 'bn' ? 'PDF তৈরি করতে ব্যর্থ। আবার চেষ্টা করুন।' : 'Failed to generate PDF. Please try again.');
    }
  };
    ...
    <div className="bg-white p-8 max-w-4xl mx-auto">
      ...
      <div className="text-center mb-8 pb-6 border-b-2 border-gray-300">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">
          {t.building.residentList || (language === 'bn' ? 'বাসিন্দা তালিকা' : 'Resident List')}
        </h1>
        ...
        <p className="text-sm text-gray-500 mt-2">
          {language === 'bn' ? 'তৈরির তারিখ' : 'Generated on'}: {currentDate}
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-8 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <div>
          <p className="text-sm text-gray-600">{language === 'bn' ? 'মোট ফ্ল্যাট' : 'Total Flats'}</p>
          <p className="text-2xl font-bold text-blue-600">{totalFlats}</p>
        </div>
        <div>
          <p className="text-sm text-gray-600">{language === 'bn' ? 'মোট বাসিন্দা' : 'Total Residents'}</p>
          <p className="text-2xl font-bold text-blue-600">{totalResidents}</p>
        </div>
        <div>
          <p className="text-sm text-gray-600">{language === 'bn' ? 'মালিকানাধীন' : 'Owned'}</p>
          <p className="text-xl font-semibold text-green-600">{ownedFlats}</p>
        </div>
        <div>
          <p className="text-sm text-gray-600">{language === 'bn' ? 'ভাড়া' : 'Rented'}</p>
          <p className="text-xl font-semibold text-orange-600">{rentedFlats}</p>
        </div>
        <div>
          <p className="text-sm text-gray-600">{language === 'bn' ? 'বসবাসকৃত' : 'Occupied'}</p>
          <p className="text-xl font-semibold text-gray-700">{occupiedFlats}</p>
        </div>
        <div>
          <p className="text-sm text-gray-600">{language === 'bn' ? 'খালি' : 'Vacant'}</p>
          ...
          <div key={floor} className="mb-6">
            ...
            <div className="bg-gray-100 px-4 py-2 rounded-t-lg border-b-2 border-gray-300">
              ...
              <p className="text-sm text-gray-600">
                {flats.length} {language === 'bn' ? 'ফ্ল্যাট' : flats.length === 1 ? 'flat' : 'flats'}
              </p>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <div className="flex items-center justify-end gap-4 p-6 border-t border-gray-200 bg-gray-50">
            ...
            >
              ...
              {language === 'bn' ? 'PDF ডাউনলোড করুন' : 'Download PDF'}
            </button>
          </div>
        </div>
      </div>

      {/* Offscreen rendering for PDF generation */}
